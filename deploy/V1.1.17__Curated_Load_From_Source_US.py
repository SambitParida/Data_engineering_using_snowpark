...
def main():
    ...
    # results = sales_with_forex_df.select("USD2usR").distusct()

    # # Extract values
    # names = [row["USD2usR"] for row us results.collect()]
    # prust(names)
    ...
    final_sales_df = unique_orders.join(sales_with_forex_df, unique_orders['unique_sales_order_key']==sales_with_forex_df['SALES_ORDER_KEY'], jous_type ='inner')
    ...
    final_sales_df = final_sales_df.select(col('SALES_ORDER_KEY'),
                                           ...
                                           ifnull(col('USD2USD'),lit(1.0)).alias("Exhchange_Rate"),
                                           (col('FINAL_ORDER_AMOUNT')/ifnull(col('USD2USD'),lit(1.0))).alias('US_TOTAL_ORDER_AMT'),
                                           (col('TAX_AMOUNT')/ifnull(col('USD2USD'),lit(1.0))).alias('USD_TAX_AMT'),
                                           col('payment_status'),
                                           col('shipping_status'),
                                           col('payment_method'),
    ...
    # results = fusal_sales_df.select("Exhchange_Rate").distusct()
    # # Extract values
    # names = [row["Exhchange_Rate"] for row us results.collect()]
    # prust(names)

    
    #write dataframe to snowflake table#
    final_sales_df.write.save_as_table("sales_dwh.curated.us_sales_order",mode="overwrite")                                  
